...
{ pkgs }:
{
  testScript = ''
    ...
    # gzip over the socket. The same file, so the stored rows must be identical to the
    # uncompressed ones — compression must not be observable in the data.
    machine.succeed(f"gzip -c {payload} > {payload}.gz")
    post_protobuf(f"{payload}.gz", extra="-H 'Content-Encoding: gzip'")
    assert row_count() == before + 6

    rows = gps_rows()
    assert len(rows) == gps_before + 2, f"expected 2 new gps rows, got {len(rows) - gps_before}"

    # Newest first, and both posts came from one file, so these are the pair just sent.
    a, b = rows[0], rows[1]
    assert a["event_time_unix_nano"] == b["event_time_unix_nano"], (a, b)
    assert a["body"] == b["body"], "gzip and identity must store identical bodies"
    assert a["attributes"] == b["attributes"], "and identical attributes"
    ...
    assert len(hits) >= 2, f"integer attribute filter returned {len(hits)} rows"
    ...
    assert row_count() == before + 6, "a refused request must not store anything"
