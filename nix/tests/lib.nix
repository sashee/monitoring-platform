...
let
  ...
  preamble =
    ...
    + ''
      ...
      def create_web_user(username=WEB_USER, password=WEB_PASSWORD):
          ...
          # create-user cannot make its own parent -- so this is callable before the receiver has ever
          # started, which the isolated cases need.
          machine.succeed(
              f"install -d -m 0700 -o {SERVICE_USER} -g {SERVICE_USER} $(dirname {DB})"
          )
      ...
      def web_login(username=WEB_USER, password=WEB_PASSWORD):
          ...
          for line in out.splitlines():
              if line.lower().startswith("set-cookie:") and "mp_session=" in line:
                  value = line.split("mp_session=", 1)[1].split(";", 1)[0]
                  # A logout response also sets this cookie, with an empty value -- so an empty one here
                  # means the login did not actually establish anything.
                  if value:
                      return value
          raise Exception(f"login did not set a session cookie:\n{out}")

      def http_status(response):
          # The status line of a `curl -i` response, as an int.
  ...
  cases = {
    ...
    web-ui = import ./cases/web-ui.nix { inherit pkgs; };
